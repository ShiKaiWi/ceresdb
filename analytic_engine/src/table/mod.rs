...
impl PendingWrites {
    ...
    fn try_push(&mut self, request: WriteRequest) -> QueueResult {
        if !self.is_same_schema(request.row_group.schema()) {
            warn!("Schema is different, reject write request");
...
impl TableImpl {
    fn trigger_flush_pending_queue(
        ...
        let _ = runtime.spawn(async move {
            ...
            info!(
                "Triggered for write, table:{}, table_id:{}, num_rows:{num_rows}",
                table_data.name, table_data.id,
            );
            let mut serial_exec = match table_data.try_acquire_serial_exec_lock("for-pending-write")
            {
                Some(v) => v,
                None => return,
            };
            info!(
                "Start to write, table:{}, table_id:{}, num_rows:{num_rows}",
                table_data.name, table_data.id,
            );
            ...
            if pending_writes.is_empty() {
                warn!("Found empty pending writes, table:{}", table_data.name);
                return;
            }
            ...
            match write_res {
                ...
                Err(e) => {
                    ...
                    error!("{err_msg}, table:{}", table_data.name);
                    for notifier in pending_writes.notifiers {
